...
demoNotification.error = function (xhr, exception, form) {
    ...
    if (contentType && contentType.indexOf('application/json') !== -1) {
        try {
            ...
            if (obj.error) {
                b5alert.error(form.closest('.form-wrapper'), obj.error, 'Error');
                return;
            }
            ...
            if (obj.errors) {
                ...
                message += '</ul>'
                b5alert.error(form.closest('.form-wrapper'), message, 'Error');
                return;
            }
        } catch (error) {
    ...
    b5alert.error(form.closest('.form-wrapper'), demoNotification.getErrorMessage(xhr, exception));
};
